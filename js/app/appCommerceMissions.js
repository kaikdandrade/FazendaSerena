"use strict";
  function renderContractDock() {
    ...
    dom.contractDock.innerHTML = `<section class="contract-dock-panel contract-dock-v2">
      ...
      <div class="contract-dock-list">${contracts.map(contract => {
        ...
        const state = progress.defaulted ? "Contrato vencido" : progress.completed ? "Concluído" : engine.formatTime(contract.timeRemaining);
  ...
  function renderContracts() {
    ...
    const offerCards = offers.map(contract => {
      ...
      const proposalTimer = `<span class="contract-time-v2 contract-proposal-time-v3" title="Tempo restante para decidir se assina"><img src="assets/icons/relogio.webp" alt="">${engine.formatTime(contract.timeRemaining)}</span>`;
