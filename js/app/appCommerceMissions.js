"use strict";
  ...
  function renderContracts() {
    ...
    const offerCards = offers.map(contract => {
      ...
      const proposalTimer = contract.offerCountdown === false ? "" : `<span class="contract-time-v2 contract-proposal-time-v3" title="Tempo restante para decidir se assina"><img src="assets/icons/relogio.webp" alt="">${engine.formatTime(contract.timeRemaining)}</span>`;
      const deliveryTime = `<div class="contract-delivery-time-v3"><span><img src="assets/icons/relogio.webp" alt="">Tempo para concluir</span><strong>${engine.formatTime(contract.durationSeconds)}</strong></div>`;
